import { button, derived, div, effect, icon, span, type Instance, baseProps, textProps } from "../../dom/factory";
import { input } from "../../dom/factory/content-ops/form";
import {
    CTRL_BLOCK,
    CTRL_ENTRY,
    CTRL_RANGE,
...
function buildRangeControl(meta: SlotMeta): Instance {
    ...
    const value = span(
        textProps(
            [RANGE_VALUE_CLASS],
            derived(() => personaStore.valueOf(meta.key) ?? defaultValueOf(meta.key) ?? String(min)),
        ),
    );
    return div(baseProps([RANGE_CLASS]), [slider, value]);
}
