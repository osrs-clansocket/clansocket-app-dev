...
async function fetchMembers(clanId: string): Promise<readonly DataSourceItem[]> {
    ...
    try {
        const response = await fetch(
            `/api/flows/data-source?source=clans:members&clan_id=${encodeURIComponent(clanId)}`,
        );
...
const rsnPicker: FormatPicker = (_schema: JSONSchemaNode, value, onChange, ctx) => {
    ...
    effect(() => {
        ...
        const select = buildGlassSelect(`rsn-${ctx.fieldName}-${ctx.operationId ?? "op"}`, opts, value);
