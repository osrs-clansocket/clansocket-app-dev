...
async function fetchWebhooks(clanId: string): Promise<readonly DataSourceItem[]> {
    const cached = cache.get(clanId);
    if (cached) return cached;
    try {
        const response = await fetch(
            `/api/flows/data-source?source=discord:webhooks&clan_id=${encodeURIComponent(clanId)}`,
        );
...
const webhookPicker: FormatPicker = (_schema: JSONSchemaNode, value, onChange, ctx) => {
    const host = div(baseProps([]));
    const optionsSignal = signal<readonly SelectOption[]>([
        { value: "", label: "Pick a webhook" },
        { value, label: value.length > 0 ? value : "—" },
    ]);
    void fetchWebhooks(ctx.clanId).then((items) => {
        const opts: SelectOption[] = [{ value: "", label: "Pick a webhook" }];
        for (const item of items) opts.push({ value: item.id, label: item.name });
        optionsSignal.set(opts);
    });
    effect(() => {
        const opts = optionsSignal();
        const select = buildGlassSelect(`wh-${ctx.fieldName}-${ctx.operationId ?? "op"}`, opts, value);
...
registerFormat("discord-webhook-id", webhookPicker);
