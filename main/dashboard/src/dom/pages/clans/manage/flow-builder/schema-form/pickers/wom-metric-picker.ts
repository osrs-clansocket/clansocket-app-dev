...
async function fetchMetrics(clanId: string): Promise<readonly DataSourceItem[]> {
    const cached = cache.get(clanId);
    if (cached) return cached;
    try {
        const response = await fetch(
            `/api/flows/data-source?source=wom:metrics&clan_id=${encodeURIComponent(clanId)}`,
        );
...
const womMetricPicker: FormatPicker = (_schema: JSONSchemaNode, value, onChange, ctx) => {
    ...
    void fetchMetrics(ctx.clanId).then((items) => {
        const opts: SelectOption[] = [{ value: "", label: "Pick a metric" }];
        for (const item of items) opts.push({ value: item.id, label: item.name });
        optionsSignal.set(opts);
    });
    effect(() => {
        const opts = optionsSignal();
        const select = buildGlassSelect(`metric-${ctx.fieldName}-${ctx.operationId ?? "op"}`, opts, value);
...
registerFormat("wom-metric", womMetricPicker);
