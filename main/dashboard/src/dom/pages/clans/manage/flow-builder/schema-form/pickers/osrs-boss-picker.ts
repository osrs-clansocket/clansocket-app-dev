...
async function fetchBosses(clanId: string): Promise<readonly DataSourceItem[]> {
    const cached = cache.get(clanId);
    if (cached) return cached;
    try {
        const response = await fetch(
            `/api/flows/data-source?source=wom:bosses&clan_id=${encodeURIComponent(clanId)}`,
        );
...
const bossPicker: FormatPicker = (_schema: JSONSchemaNode, value, onChange, ctx) => {
    const host = div(baseProps([]));
    const optionsSignal = signal<readonly SelectOption[]>([
        { value: "", label: "Pick a boss" },
        { value, label: value.length > 0 ? value : "—" },
    ]);
    void fetchBosses(ctx.clanId).then((items) => {
        const opts: SelectOption[] = [{ value: "", label: "Pick a boss" }];
        for (const item of items) opts.push({ value: item.id, label: item.name });
        optionsSignal.set(opts);
    });
    effect(() => {
        const opts = optionsSignal();
        const select = buildGlassSelect(`boss-${ctx.fieldName}-${ctx.operationId ?? "op"}`, opts, value);
...
registerFormat("osrs-boss", bossPicker);
