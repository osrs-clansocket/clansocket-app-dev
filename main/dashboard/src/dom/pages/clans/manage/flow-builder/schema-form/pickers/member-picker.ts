...
async function fetchMembers(clanId: string): Promise<readonly DataSourceItem[]> {
    const cached = cache.get(clanId);
    if (cached) return cached;
    try {
        const response = await fetch(
            `/api/flows/data-source?source=discord:members&clan_id=${encodeURIComponent(clanId)}`,
        );
...
const memberPicker: FormatPicker = (_schema: JSONSchemaNode, value, onChange, ctx) => {
    ...
    void fetchMembers(ctx.clanId).then((items) => {
        const opts: SelectOption[] = [{ value: "", label: "Pick a member" }];
        for (const item of items) opts.push({ value: item.id, label: `@${item.name}` });
        optionsSignal.set(opts);
    });
    effect(() => {
        const opts = optionsSignal();
        const select = buildGlassSelect(`mem-${ctx.fieldName}-${ctx.operationId ?? "op"}`, opts, value);
...
registerFormat("discord-member-id", memberPicker);
