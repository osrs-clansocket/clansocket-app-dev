...
import { getRunewatchStore } from "../../../../../state/clans/runewatch/runewatch-store.js";
import type { Instance } from "../../../../factory";
import { brandHead, buildBottomPane, buildTopPane } from "./index-panes.js";
import {
    makeBottomRenderer,
    makeRefreshFn,
    makeSentinelObserver,
    makeTopRenderer,
    type RunewatchState,
...
function buildRunewatchTab(slug: string): HTMLElement {
    ...
    void store.ensure();
    w.refreshRef.fn = makeRefreshFn(slug, store, w.top.refreshBtn);
    const offRoute = events.on("route:change", () => {
        dispose.dispose();
        observer.disconnect();
        offRoute();
    });
    const host = div({ classes: [HOST_CLASS, SHELL_CLASS], context: null, meta: null });
    host.setChildren(brandHead(), w.top.topPane, w.bottom.bottomPane);
    return host.el;
}

defineManageTab({ key: "runewatch", build: (slug) => buildRunewatchTab(slug), order: 60 });

export { buildRunewatchTab };
