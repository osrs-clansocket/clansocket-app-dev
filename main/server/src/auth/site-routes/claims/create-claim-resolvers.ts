...
export async function validateGateClaim(rsn: unknown, res: Response): Promise<string | null> {
    if (typeof rsn !== "string" || rsn.trim().length === 0 || rsn.trim().length > RSN_MAX_LEN) {
        res.status(HTTP_BAD_REQUEST).json(failBody(CLAIM_REASON_BAD_PAYLOAD, CLAIM_MESSAGE_BAD_PAYLOAD));
        return null;
    }
    const trimmedRsn = rsn.trim();
    const gate = await gateRsnSubmission(trimmedRsn);
    if (!gate.ok) {
        res.status(HTTP_FORBIDDEN).json(failBody(gate.reason, gate.message));
        return null;
    }
    return trimmedRsn;
}
...
function respondNoLive(res: Response): null {
    res.status(HTTP_BAD_REQUEST).json(failBody(CLAIM_REASON_NO_LIVE_PLUGIN, CLAIM_MESSAGE_NO_LIVE_PLUGIN));
    return null;
}
