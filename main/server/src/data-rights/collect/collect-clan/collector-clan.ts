...
const CLAN_DB_COLLECTORS: ReadonlyArray<{
    file: string;
    collect: (clanId: string, entries: ZipEntry[], summary: ClanCollectionSummary) => void;
}> = [
    { file: "clan.db", collect: collectClanDb },
    { file: "clan_audit.db", collect: collectClanAudit },
];
...
function collectClanFiles(clanId: string, clan: ClanRow, entries: ZipEntry[], summary: ClanCollectionSummary): void {
    ...
    for (const { file, collect } of CLAN_DB_COLLECTORS) {
        if (existsSync(resolve(clanDir, file))) collect(clanId, entries, summary);
    }
