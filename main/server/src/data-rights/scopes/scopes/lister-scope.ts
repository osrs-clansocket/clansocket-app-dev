...
import type { ClanPresence, ScopeListItem } from "./types.js";
import { appTables, buildTableList } from "./table-presence.js";

interface BaseScopeDef {
    scope: Scope;
    label: string;
    tables: () => string[];
}

const baseDef = (scope: Scope, label: string, tables: () => string[]): BaseScopeDef => ({ scope, label, tables });

const BASE_SCOPE_DEFS: ReadonlyArray<BaseScopeDef> = [
    baseDef({ kind: SCOPE_APP }, "Account", () => appTables()),
    baseDef({ kind: SCOPE_VAREZ }, "AI history", () => VAREZ_TABLES_BY_SITE_ACCOUNT.map((e) => e.table)),
];

function baseScopes(siteAccountId: string): ScopeListItem[] {
    return BASE_SCOPE_DEFS.map((d) => ({
        kind: d.scope.kind,
        label: d.label,
        tables: buildTableList(siteAccountId, d.scope, d.tables()),
    }));
}
